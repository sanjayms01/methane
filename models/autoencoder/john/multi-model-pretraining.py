...
####################################################################
def standardize_data(train, validation, test, feature_cols, describe=False, save=False):
    ...
    if save:
        ...
        standard_scaler_name = f'ScalerModel_{cur_date}'                              
        pickle.dump(scaler, open(f'/root/methane/models/autoencoder/pretrained/{standard_scaler_name}.pkl','wb'))
...
for zone in zones:
    ...
    #standardize data
    train_scaled, val_scaled, test_scaled, scaler = standardize_data(train_zone, val_zone, test_zone, feature_cols, save=True)
    ...
    import datetime
    cur_date = datetime.datetime.now().strftime("%Y%m%d")
    model_name = f'LSTMAE_Zone{zone}_{cur_date}'                              
    print("saving model named:", model_name)
    model.save(f'/root/methane/models/autoencoder/pretrained/{model_name}.h5')   
...
import subprocess
subprocess.check_output(['aws','s3','cp', '--recursive', '../pretrained' , 's3://methane-capstone/models/autoencoder/pretrained/'])
