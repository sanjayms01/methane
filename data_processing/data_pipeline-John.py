...
# Steps:  
# 1. Define date range (`start_dt` and `end_dt`).

# +
from datetime import timedelta, date

start_dt = date(2020, 1, 31 )          #DEFINE START DATE HERE
end_dt = date(2020, 2, 1)              #DEFINE END DATE HERE
...
# Step 3: Merge each date's data into one dataframe. Save to S3.  
#
...
subfolder = 'data/pipeline-raw-data'
s3_path = bucket+'/'+subfolder 
file_name='2020-10-31_2020-11-01_combined_data.parquet.gzip'
data_location = 's3://{}/{}'.format(s3_path, file_name)
...
day=""
new_dataset[new_dataset['time_utc']== day].sort_values(by='BZone').head()
